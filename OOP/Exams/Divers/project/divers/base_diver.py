...
class BaseDiver(ABC):
    ...
    def __str__(self):
        return f"{type(self).__name__}: [Name: {self.name}, Oxygen level left: {self.oxygen_level}, Fish caught: {len([f.name for f in self.catch])}, Points earned: {self.competition_points}]"
